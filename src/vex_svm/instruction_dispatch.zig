    ...
    fn trampoline(
        ...
        switch (kind) {
            .vote => {
                ...
                // SIMD-0387: no shared per-tx CU meter is threaded into this CPI
                // trampoline today (pre-existing gap, out of scope for the
                // top-level-instruction CU-parity fix) — `null` preserves prior
                // behavior (PoP crypto enforced, no CU gate) rather than
                // fabricating an unrelated counter. Flagged as a follow-up.
                executeVoteInstruction(fb.ix, fb.ptx, fb.bank, fb.db, alloc, fb.ancestor_slots, null) catch {
...
pub fn executeVoteInstruction(
    ...
    ancestor_slots: []const u64,
    // SIMD-0387 CU-parity fix (2026-07-26): the caller's shared per-tx CU
    // meter (the same one already charged the flat 2,100 builtin entry cost
    // one level up), threaded to `voteforge.ExecContext.pop_cu_meter` so the
    // 34,500 BLS-PoP surcharge draws from the identical pool Agave's
    // `consume_pop_compute_units` closure closes over. `null` where no such
    // meter exists at the call site (see call-site comments).
    cu_meter: ?*u64,
) !void {
...
pub fn executeVoteViaVoteforge(
    ...
    append_fail: *u32,
    ok: *u32,
    // SIMD-0387 CU-parity fix — see `executeVoteInstruction`'s param doc.
    cu_meter: ?*u64,
) !void {
    ...
    var ctx = vi.ExecContext{
        ...
        .alloc = alloc,
        // SIMD-0387 CU-parity fix — threaded straight from the caller's shared
        // per-tx meter; see `executeVoteInstruction`'s param doc.
        .pop_cu_meter = cu_meter,
    };
