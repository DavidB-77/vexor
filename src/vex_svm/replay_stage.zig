    ...
    for (ctx.tx_indices) |tx_idx| {
        ...
        for (ptx.instructions[0..ptx.num_instructions]) |ix| {
            ...
            if (std.mem.eql(u8, program_id, &NATIVE_PROGRAM_IDS.SYSTEM)) {
                ...
                // SIMD-0387: dead path, no CU meter to thread — null.
                executeVoteInstruction(ix, ptx, ctx.bank, ctx.db, ctx.alloc, &[_]u64{}, null) catch {};
    ...
    fn executeDagTx(
        ...
        for (ptx.instructions[0..ptx.num_instructions], 0..) |ix, ix_idx| {
            ...
            if (std.mem.eql(u8, program_id, &NATIVE_PROGRAM_IDS.SYSTEM)) {
                ...
                // SIMD-0387 CU-parity fix (2026-07-26): thread THIS tx's shared
                // meter so a VoterWithBLS/InitializeAccountV2 PoP charge (34,500,
                // on top of the flat 2,100 already drawn above) can abort the ix
                // before the crypto check, matching Agave's `consume_pop_compute
                // _units` closure over the same `invoke_context.compute_meter`.
                executeVoteInstruction(ix, ptx, bank, db, arena, ancestor_slots, &tx_cus_remaining) catch |e| {
                                ...
                                if (!verifyTxPrecompiles(arena_alloc, &ptx, bank.slot, self.live_feature_set)) {
                                    ...
                                    for (ptx.instructions[0..ptx.num_instructions], 0..) |ix, ix_idx_serial| {
                                        ...
                                        if (std.mem.eql(u8, program_id, &NATIVE_PROGRAM_IDS.SYSTEM)) {
                                            ...
                                            // SIMD-0387 CU-parity fix (2026-07-26): see DAG twin above.
                                            executeVoteInstruction(ix, &ptx, bank, self.accounts_db.?, arena_alloc, ancestor_slots, &tx_cus_remaining) catch |e| {
