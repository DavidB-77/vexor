...
fn solMemcmp(ctx: *VmContext, a_vm: u64, b_vm: u64, n: u64, out_vm: u64, _: u64) VmError!u64 {
    ...
    var cmp: i32 = 0;
    for (0..n) |i| {
        if (a[i] != b[i]) { cmp = @as(i32, a[i]) - @as(i32, b[i]); break; }
    }
...
const ClockLayout = extern struct {
    slot: u64, epoch_start_ts: i64, epoch: u64, leader_schedule_epoch: u64, unix_ts: i64,
};
...
fn solGetClock(ctx: *VmContext, out_vm: u64, _: u64, _: u64, _: u64, _: u64) VmError!u64 {
    ...
    const clk = ClockLayout{
        .slot = 0, .epoch_start_ts = now, .epoch = 0,
        .leader_schedule_epoch = 1, .unix_ts = now,
    };
...
const EpochSchedLayout = extern struct {
    ...
    warmup: bool, _pad: [7]u8, first_normal_epoch: u64, first_normal_slot: u64,
};
...
fn solGetEpochSched(ctx: *VmContext, out_vm: u64, _: u64, _: u64, _: u64, _: u64) VmError!u64 {
    ...
    const es = EpochSchedLayout{
        .slots_per_epoch = 432_000, .leader_schedule_slot_offset = 432_000,
        .warmup = false, ._pad = [_]u8{0}**7, .first_normal_epoch = 0, .first_normal_slot = 0,
    };
