...
pub const Vm = struct {
                    ...
                    sbpf.ALU_END => {
                        ...
                        self.regs[d] = switch (imm) {
                            16 => blk: {
                                ...
                                break :blk if (want_be) std.mem.nativeToBig(u16, v16) else std.mem.nativeToLittle(u16, v16);
                            },
                            32 => blk: {
                                const v32: u32 = @truncate(self.regs[d]);
                                break :blk if (want_be) std.mem.nativeToBig(u32, v32) else std.mem.nativeToLittle(u32, v32);
                            },
                            64 => if (want_be) std.mem.nativeToBig(u64, self.regs[d]) else std.mem.nativeToLittle(u64, self.regs[d]),
    ...
    fn doCall(self: *Self, ver: Version, inst: Instruction, pc: u64) ExecutionError!void {
        ...
        if (!is_call_reg) {
            ...
            // @prov:vm.module-map
            const s = if (ver.callRegUsesSrc()) @as(usize, inst.src) else @as(usize, inst.imm & 0x0f);
...
test "vm: mov64 + exit" {
    ...
    const bytecode = [_]Instruction{
        .{ .opcode = sbpf.CLS_ALU64 | sbpf.ALU_MOV | sbpf.SRC_IMM, .dst = 0, .src = 0, .off = 0, .imm = 42 },
